...
public class CommandRun {
     ...
     */
    public static ArrayList<CardInput> getPlayerDeck(int playerIndex, GameInput game) {
        if (playerIndex == 1) {
     ...
     */
    public static int getPlayerTurn(GameInput game) {
     ...
     */
    public static CardInput getCardAtPosition(GameInput game, int x, int y) {
        Board board = game.getStartGame().getBoard();
     ...
     */
    public static ArrayList<CardInput> getEnvironmentCardsInHand(GameInput game, int playerIndex) {
     ...
     */
    public static ArrayNode getFrozenCardsOnTable(GameInput game) {
        Board board = game.getStartGame().getBoard();
     ...
     * @return error String or empty String if no error
     */
    public static String useEnvironmentCard(GameInput game, int cardIndex, int rowIndex) {
     ...
     * @return error String or empty String if no error
     */
    public static String cardUsesAbility(GameInput game, Coordinates coords1, Coordinates coords2) {
        Board board = game.getStartGame().getBoard();
     ...
     * @return error String or empty String if no error
     */
    public static String useAttackHero(GameInput game, Coordinates coords1) {
        Board board = game.getStartGame().getBoard();
        CardInput card = board.getCard(coords1);
        if (card == null) {
        ...
        HeroCard hero;
        if (game.getCurrentPlayer().getIndex() == 1) hero = (HeroCard) game.getStartGame().getPlayerTwoHero();
        else hero = (HeroCard) game.getStartGame().getPlayerOneHero();
        card.useAttack(hero);
        return "";
    }
     ...
     */
    public static String useHeroAbility(GameInput game, int rowIndex) {
        Board board = game.getStartGame().getBoard();
        Player player = game.getCurrentPlayer();
        HeroCard hero;
        if (player.getIndex() == 1) hero = (HeroCard) game.getStartGame().getPlayerOneHero();
        else hero = (HeroCard) game.getStartGame().getPlayerTwoHero();
    ...
    public static ObjectNode run(Input input, GameInput game, ActionsInput action) {
        ...
        switch (action.getCommand()) {
            ...
            case "endPlayerTurn" -> {
                game.getStartGame().getBoard().unfrozePlayerCards(game.getCurrentPlayer().getIndex());
            ...
            case "cardUsesAttack" -> {
                out = new AttackCardError(action.getCommand(), action.getCardAttacker(), action.getCardAttacked(),
                        CommandRun.cardUsesAttack(game, action.getCardAttacker(), action.getCardAttacked()));
                if (out.getError().equals("")) break;
                return out.convertToObjectNode();
            }
            ...
            case "useEnvironmentCard" -> {
                out = new UseEnvironmentCardError(action.getCommand(), action.getHandIdx(), action.getAffectedRow(),
                        CommandRun.useEnvironmentCard(game, action.getHandIdx(), action.getAffectedRow()));
                if (out.getError().equals("")) break;
                return out.convertToObjectNode();
            }
            case "cardUsesAbility" -> {
                out = new AttackCardError(action.getCommand(), action.getCardAttacker(), action.getCardAttacked(),
                        CommandRun.cardUsesAbility(game, action.getCardAttacker(), action.getCardAttacked()));
                if (out.getError().equals("")) break;
                return out.convertToObjectNode();
            }
            case "useAttackHero" -> {
